...
export default function TableItem({ id, status, col1, col2, col3, col4 ,tipo, onClickEdit, onClickDelete}) {
        ...
        <div className="table-item">
            ...
            <div className={`table-item-status ${status === 'ativo' ? 'active' : 'inative'}`} to="/"></div>
                <div className="table-item-column">
                    <Link to={`/condominio/${id}`}>{truncate(col1,60)}</Link>
                </div>
                ...
                <div className="table-item-action">
                    <button className="button-action view" onClick={onClickEdit}>
